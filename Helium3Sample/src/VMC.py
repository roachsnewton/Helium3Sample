...
####################################################################################
# MALA: sampling with quantum force
...
@partial(jax.pmap, axis_name="p",
                   in_axes=(0, 
                            ...
                            None,
                            None, None, None),
                   static_broadcasted_argnums=(1, 3, 6))
def sample_stateindices_and_x_mala(key,
                              sampler, params_van,
                              logp, x, params_flow,
                              quantum_force,
    ...
    x, accept_rate = mala(lambda x: logp(x, params_flow, state_indices), 
                          lambda x: quantum_force(x, params_flow, state_indices), 
                          x, key_MCMC, mc_steps, mc_stddev)
    x -= L * jnp.floor(x/L)
...
@partial(jax.pmap, axis_name="p",
                   ...
                   donate_argnums=4)
def sample_stateindices_and_x_mcmc(key,
                              sampler, params_van,
                              logp, x, params_flow,
                              mc_steps, mc_stddev, L):
    """
...
def make_loss(log_prob, logpsi, logpsi_grad_laplacian, 
              lamb, L, beta, params_potential, clip_factor):
    ...
    def observable_and_lossfn(params_van, params_flow, state_indices, x, key):
        ...
        # Change the energy unit into Kelvin (K)
        kinetic = lamb * (-laplacian - (grad**2).sum(axis=(-2, -1)))
        potential = potential_energy(x, L, params_potential)
