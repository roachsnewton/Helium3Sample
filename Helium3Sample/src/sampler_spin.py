...
def make_autoregressive_sampler_spin(network, sp_indices, nup, ndn, num_states, mask_fn=False):
    ...
    def _mask(state_idx):
        ...
        # mask: spin down
        maskdn = jnp.tril(jnp.ones((ndn, num_states)), k=num_states-ndn)
        idx_lb = jnp.concatenate( (jnp.array([-1]), state_idx_down[:-1]) )
        maskdn = jnp.where(jnp.arange(num_states) > idx_lb[:, None], maskdn, 0.)

        mask = jnp.concatenate((maskup, maskdn), axis = 0)
        return mask
